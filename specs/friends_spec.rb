...
class TestFriends < MiniTest::Test
  ...
  def test_likes_to_eat_daphne_spinach
    result = likes_to_eat(@person5,"spinach")
    assert_equal(true, result)
  end

  def test_likes_to_eat_fred_chips
    result = likes_to_eat(@person4,"chips")
  ...
  def test_add_friend_gordon_to_scooby
    friendsbefore = @person3[:friends].length
    add_friend(@person3,"gordon")
    friendsafter = @person3[:friends].length
    assert_equal(1, (friendsafter - friendsbefore) )
  end
  ...
  def test_remove_friend_velma_from_shaggy
    friendsbefore = @person1[:friends].length
    remove_friend(@person1,"Velma")
    friendsafter = @person1[:friends].length
    assert_equal(-1, (friendsafter - friendsbefore) )
  end
